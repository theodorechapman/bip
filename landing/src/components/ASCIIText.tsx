...
class AsciiFilter {
  ...
  constructor(renderer: THREE.WebGLRenderer, { fontSize = 12, fontFamily = "'Courier New', monospace", charset = " .\'`^\",:;Il!i~+_-?][}{1)(|/tfjrxnuvczXYUJCLQ0OZmwqpdbkhao*#MW&8%B@$", invert = true }: AsciiFilterOptions = {}) {
...
class CanvasTxt {
  ...
  resize() {
    ...
    const textHeight =
      Math.ceil((metrics.actualBoundingBoxAscent ?? 0) + (metrics.actualBoundingBoxDescent ?? 0)) + 20;
  ...
  render() {
    ...
    const metrics = this.context.measureText(this.txt);
    const yPos = 10 + (metrics.actualBoundingBoxAscent ?? 0);
...
class CanvAscii {
  ...
  setRenderer() {
    ...
    this.renderer.setPixelRatio(Math.min(PX_RATIO, 2));
  ...
  clear() {
    this.scene.traverse((obj) => {
      ...
      if (mesh.isMesh && typeof mesh.material === 'object' && mesh.material !== null) {
        ...
        if (typeof material === 'object' && typeof material.dispose === 'function') {
          material.dispose();
        }
